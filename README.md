...
Phase 1's exit gate was a kill condition and it passed at the time — but those figures predate an audit that fixed 82 defects in the code producing them, and they have not been re-verified. See [VALIDATION.md](VALIDATION.md) for the measurement and [ROADMAP-2.md](ROADMAP-2.md) for what is and is not currently established. Phase 2 added the scope gate, certificate inventory with a lifetime-vs-deadline check, TLS/SSH capability enumeration, and cloud key-store classification. Its exit gate — that an out-of-scope probe fails at the type level rather than at runtime — is asserted by a compile-time test that runs under `tsc` in CI.
