/**
 * Pure SHA-256. Zero dependencies, no node builtins, isomorphic.
 *
 * @assay/core must be importable from the browser bundle and must have no
 * runtime dependency beyond zod (I7). Asset identity is a content hash, and a
 * content hash that differs between server and browser would break dedupe, so
 * it lives here rather than in node:crypto.
 */
